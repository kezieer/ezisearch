...
function fetchUrl(string $url)
{
    if (function_exists('curl_init')) {
        ...
        curl_setopt_array($handle, [
            ...
            CURLOPT_TIMEOUT => 6,
            CURLOPT_USERAGENT => 'EziSearch News Reader/1.0',
        ]);
    ...
    $context = stream_context_create([
        'http' => [
            'timeout' => 6,
            'user_agent' => 'EziSearch News Reader/1.0',
        ],
    ]);
...
function cleanSummary(string $description): string
{
    $cleanText = trim(preg_replace('/\s+/', ' ', strip_tags(html_entity_decode($description, ENT_QUOTES, 'UTF-8'))));

    if (strlen($cleanText) <= 220) {
        return $cleanText;
    }

    return substr($cleanText, 0, 217) . '...';
}

function loadNewsItems(string $query, int $limit = 12): array
{
    if ($query === '' || !function_exists('simplexml_load_string')) {
        return [];
    }
    ...
    foreach ($rss->channel->item as $item) {
        ...
        $source = isset($item->source) ? trim((string) $item->source) : parse_url($link, PHP_URL_HOST);

        $items[] = [
            'title' => $title,
            'link' => $link,
            'source' => $source ?: 'News source',
            'summary' => cleanSummary((string) $item->description),
...
<head>
    ...
    <title><?= $query === '' ? 'News results' : e($query) . ' news - EziSearch' ?></title>
...
<body>
    <header class="topbar">
        ...
        <form class="search-form" action="news.php" method="get" role="search">
            <input class="search-input" id="searchInput" type="search" name="q" value="<?= e($query) ?>" autocomplete="off" list="searchRecommendations" aria-label="Search news">
            <datalist id="searchRecommendations">
                <option value="Philippines latest news">
                <option value="Esports news today">
                <option value="Valorant news">
                <option value="Mobile Legends news">
                <option value="NBA trade news">
                <option value="LeBron James news">
                <option value="Technology news">
                <option value="Business news Philippines">
                <option value="AI news 2026">
                <option value="World news today">
            </datalist>
            ...
            <button class="search-button" type="submit" aria-label="Search news">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
    ...
    <nav class="tabs" aria-label="Result types">
        ...
        <a class="tab is-active" href="<?= buildNewsUrl($query) ?>">News</a>
    ...
    <main class="news-shell">
        ...
        <?php if ($query === ''): ?>
            <section class="empty-state">
                <h1>Search news</h1>
                <p>Type or speak a topic above to see connected news articles.</p>
            </section>
        <?php else: ?>
            <p class="meta">News results for <?= e($query) ?></p>
                ...
                <?php foreach ($newsItems as $item): ?>
                    <article class="news-card">
                        ...
                        <?php if ($item['timestamp']): ?>
                            <span class="news-date"> · <?= e(date('M j, Y g:i A', $item['timestamp'])) ?></span>
                        <?php endif; ?>
                        <a class="news-title" href="<?= e($item['link']) ?>" target="_blank" rel="noopener noreferrer"><?= e($item['title']) ?></a>
                        <?php if ($item['summary'] !== ''): ?>
            ...
            <?php else: ?>
                <section class="empty-state">
                    ...
                    <p>News articles could not load right now. Please check your connection or try another search.</p>
                </section>
            <?php endif; ?>
        <?php endif; ?>
    </main>
